...
NR_event<User_Data>* Event_Manager<User_Data>::Set_user_event(Callback<Data*, User_Data*> *_function, Uint32 _key, User_Data *_user_data)
{
	NR_event<User_Data> *newEvent = new NR_event<User_Data>();
...
void Event_Manager<class User_Data>::Cheack(Data *_data, User_Data *_user_data)
{
	...
	while (SDL_PollEvent(&event)){
		...
		for (i = 0; i < n; i++)
			if ((User_Event_List[i]->run) && (User_Event_List[i]->key == event.key.keysym.sym)) User_Event_List[i]->function->operator()(_data, _user_data);
		if (event.type == SDL_MOUSEMOTION){
			_data->mouse_motion_x = event.motion.xrel;
			_data->mouse_motion_y = event.motion.yrel;

			for (i = 0; i < n; i++)
				if ((User_Event_List[i]->run) && (User_Event_List[i]->key == SDL_MOUSEMOTION)) User_Event_List[i]->function->operator()(_data, _user_data);
		}
	...
	n = Eventlist.size();
	for (i = 0; i < n; i++)
		if ((Eventlist[i]->timer) && (Eventlist[i]->run) && (_data->_tick % Eventlist[i]->timer == 0)) Eventlist[i]->function->operator()(_data, _user_data);
